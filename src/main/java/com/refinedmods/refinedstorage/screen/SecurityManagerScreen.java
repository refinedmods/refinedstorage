...
public class SecurityManagerScreen extends BaseScreen<SecurityManagerContainer> {
    ...
    public void renderForeground(MatrixStack matrixStack, int mouseX, int mouseY) {
        ...
        renderString(matrixStack, 7, 59, new TranslationTextComponent("gui.refinedstorage.security_manager.configure").getString());
        renderString(matrixStack, 7, 140, new TranslationTextComponent("container.inventory").getString());
        ...
        for (int i = 0; i < permissions.length; ++i) {
            ...
            if (RenderUtils.inBounds(permission.x - guiLeft, permission.y - guiTop, permission.getWidth(), permission.getHeight(), mouseX, mouseY)) {
                renderTooltip(matrixStack, mouseX, mouseY, new TranslationTextComponent("gui.refinedstorage.security_manager.permission." + i + ".tooltip").getString());
            }
        }
    }
}
