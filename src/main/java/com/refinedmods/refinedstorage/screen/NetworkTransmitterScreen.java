...
public class NetworkTransmitterScreen extends BaseScreen<NetworkTransmitterContainer> {
    ...
    public void renderForeground(MatrixStack matrixStack, int mouseX, int mouseY) {
        ...
        if (!receiverDim.isPresent()) {
            text = new TranslationTextComponent("gui.refinedstorage.network_transmitter.missing_card").getString();
        } else if (distance != -1) {
            text = new TranslationTextComponent("gui.refinedstorage.network_transmitter.distance", distance).getString();
        } else {
            text = receiverDim.get().toString();
        }

        renderString(matrixStack, 51, 24, text);
        renderString(matrixStack, 7, 42, new TranslationTextComponent("container.inventory").getString());
    }
}