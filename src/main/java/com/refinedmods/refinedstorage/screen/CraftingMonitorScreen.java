...
public class CraftingMonitorScreen extends BaseScreen<CraftingMonitorContainer> {
    public static class Task implements IGridTab {
        ...
        public void drawTooltip(MatrixStack matrixStack, int x, int y, int screenWidth, int screenHeight, FontRenderer fontRenderer) {
            List<ITextProperties> textLines = Lists.newArrayList(requested.getItem() != null ? requested.getItem().getDisplayName() : requested.getFluid().getDisplayName());
            ...
            smallTextLines.add(new TranslationTextComponent("gui.refinedstorage.crafting_monitor.tooltip.requested", requested.getFluid() != null ? API.instance().getQuantityFormatter().formatInBucketForm(qty) : API.instance().getQuantityFormatter().format(qty)).getString());
    ...
    public void renderForeground(MatrixStack matrixStack, int mouseX, int mouseY) {
        ...
        if (itemSelectedTooltip != null && !itemSelectedTooltip.isEmpty()) {
            renderTooltip(matrixStack, mouseX, mouseY, new TranslationTextComponent(itemSelectedTooltip).getString());
        }
