...
public class ExactModeSideButton extends SideButton {
    ...
    public String getTooltip() {
        String tooltip = new TranslationTextComponent("sidebutton.refinedstorage.exact_mode").getString() + "\n" + TextFormatting.GRAY;

        if ((parameter.getValue() & MASK) == MASK) {
            tooltip += new TranslationTextComponent("sidebutton.refinedstorage.exact_mode.on").getString();
        } else {
            tooltip += new TranslationTextComponent("sidebutton.refinedstorage.exact_mode.off").getString();
        }
