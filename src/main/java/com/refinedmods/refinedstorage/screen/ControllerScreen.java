...
public class ControllerScreen extends BaseScreen<ControllerContainer> {
    ...
    public void renderForeground(MatrixStack matrixStack, int mouseX, int mouseY) {
        ...
        renderString(matrixStack, 7, 87, new TranslationTextComponent("container.inventory").getString());
        ...
        if (hoveringNode != null) {
            renderTooltip(matrixStack, mouseX, mouseY, new TranslationTextComponent("misc.refinedstorage.energy_usage_minimal", hoveringNode.getEnergyUsage()).getString());
        }

        if (RenderUtils.inBounds(ENERGY_BAR_X, ENERGY_BAR_Y, ENERGY_BAR_WIDTH, ENERGY_BAR_HEIGHT, mouseX, mouseY)) {
            renderTooltip(matrixStack, mouseX, mouseY, new TranslationTextComponent("misc.refinedstorage.energy_usage", ControllerTile.ENERGY_USAGE.getValue()).getString() + "\n" + new TranslationTextComponent("misc.refinedstorage.energy_stored", ControllerTile.ENERGY_STORED.getValue(), ControllerTile.ENERGY_CAPACITY.getValue()).getString());
        }
    }
