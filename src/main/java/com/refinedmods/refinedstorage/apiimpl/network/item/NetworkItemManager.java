    ...
    public void open(PlayerEntity player, ItemStack stack, int slotId) {
        ...
        for (INetworkNode node : network.getNodeGraph().all()) {
            if (node instanceof IWirelessTransmitter &&
                network.canRun() &&
                node.isActive() &&
                ((IWirelessTransmitter) node).getDimension() == player.world.func_234923_W_()) {
        ...
        if (!inRange) {
            player.sendMessage(new TranslationTextComponent("misc.refinedstorage.network_item.out_of_range"), Util.DUMMY_UUID);
