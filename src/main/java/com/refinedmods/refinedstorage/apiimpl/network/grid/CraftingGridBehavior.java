                ...
                if (!slot.isEmpty() && slot.getCount() > 1) {
                    if (!player.inventory.addItemStackToInventory(remainder.get(i).copy())) { // If there is no space in the player inventory, try to dump it in the network.
                        ...
                        if (!remainderStack.isEmpty()) {
                            InventoryHelper.spawnItemStack(player.getEntityWorld(), player.serverPosX, player.serverPosY, player.serverPosZ, remainderStack);
                        }
                    }
        ...
        for (ItemStack craftedItem : craftedItemsList) {
            if (!player.inventory.addItemStackToInventory(craftedItem.copy())) {
                ...
                if (!remainder.isEmpty()) {
                    InventoryHelper.spawnItemStack(player.getEntityWorld(), player.serverPosX, player.serverPosY, player.serverPosZ, remainder);
                }
            }
        }
