...
public class Ingredient<T> {
    ...
    public Ingredient(boolean isItem, CompoundNBT nbt) throws CraftingTaskReadException {
        ...
        if (isItem) {
            ingredients = (IStackList<T>) SerializationUtil.readItemStackList(nbt.getList(NBT_INGREDIENTS, Constants.NBT.TAG_COMPOUND));
        } else {
            ingredients = (IStackList<T>) SerializationUtil.readFluidStackList(nbt.getList(NBT_INGREDIENTS, Constants.NBT.TAG_COMPOUND));
        }
        List<Integer> ints = Ints.asList(nbt.getIntArray(NBT_AMOUNTS_PER_SLOT));
        for (int i = 0; i < ints.size(); i += 2) {
            slotCounts.put(ints.get(i), ints.get(i + 1));
        }
    }
    ...
    public void increaseCount(int slot, int count) {
        this.count += count;
        slotCounts.put(slot, count);
    }
