...
public class FluidCraftingPreviewElement implements ICraftingPreviewElement<FluidStack> {
    ...
    public void draw(MatrixStack matrixStack, int x, int y, IElementDrawers drawers) {
        ...
        if (getToCraft() > 0) {
            ...
            drawers.getStringDrawer().draw(matrixStack, RenderUtils.getOffsetOnScale(x + 23, scale), RenderUtils.getOffsetOnScale(y, scale), new TranslationTextComponent(format, API.instance().getQuantityFormatter().formatInBucketForm(getToCraft())).getString());

            y += 7;
        }

        if (getAvailable() > 0) {
            drawers.getStringDrawer().draw(matrixStack, RenderUtils.getOffsetOnScale(x + 23, scale), RenderUtils.getOffsetOnScale(y, scale), new TranslationTextComponent("gui.refinedstorage.crafting_preview.available", API.instance().getQuantityFormatter().formatInBucketForm(getAvailable())).getString());
        }
