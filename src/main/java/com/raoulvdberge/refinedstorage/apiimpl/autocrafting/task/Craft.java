...
public abstract class Craft {
    ...
    private Collection<ItemStack> defaultSet = new ArrayList<>();
    ...
    public Craft(INetwork network, NBTTagCompound tag) throws CraftingTaskReadException {
        ...
        for (int i = 0; i < list.tagCount(); i++) {
            ...
            for (int j = 0; j < list2.tagCount(); j++) {
                ...
                addToOredictStorage(StackUtils.deserializeStackFromNbt(compound.getCompoundTag(NBT_ITEM_STORED)), compound.getInteger(NBT_NUMBER_STORED), countInRecipe, ingredientNumber);
            }
        }
        defaultSet = getNextSet(Action.SIMULATE);
    }
    ...
    public Collection<ItemStack> getDefaultSet() {
        return defaultSet;
    }
    ...
    public void addToOredictStorage(ItemStack stack, int count, int countInRecipe, int ingredientNumber) {
