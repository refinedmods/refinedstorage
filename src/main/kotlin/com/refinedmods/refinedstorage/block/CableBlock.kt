...
//                ?: false
    ...
    companion object {
        ...
        protected fun getCableShape(state: BlockState): VoxelShape {
            ...
            if (state.get(NORTH)) {
                shape = VoxelShapes.combine(shape, SHAPE_NORTH, BooleanBiFunction.OR)
            }
            if (state.get(EAST)) {
                shape = VoxelShapes.combine(shape, SHAPE_EAST, BooleanBiFunction.OR)
            }
            if (state.get(SOUTH)) {
                shape = VoxelShapes.combine(shape, SHAPE_SOUTH,BooleanBiFunction.OR)
            }
            if (state.get(WEST)) {
                shape = VoxelShapes.combine(shape, SHAPE_WEST,BooleanBiFunction.OR)
            }
            if (state.get(UP)) {
                shape = VoxelShapes.combine(shape, SHAPE_UP,BooleanBiFunction.OR)
            }
            if (state.get(DOWN)) {
                shape = VoxelShapes.combine(shape, SHAPE_DOWN,BooleanBiFunction.OR)
            }
            return shape
        }
    }
