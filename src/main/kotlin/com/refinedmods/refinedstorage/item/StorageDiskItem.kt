...
class StorageDiskItem(type: ItemStorageType) : Item(Properties().group(RS.MAIN_GROUP).maxStackSize(1)), IStorageDiskProvider {
    private val type: ItemStorageType
    override fun inventoryTick(stack: ItemStack, world: World, entity: Entity, slot: Int, selected: Boolean) {
        super.inventoryTick(stack, world, entity, slot, selected)
        if (!world.isClient && !stack.hasTag()) {
            val id = UUID.randomUUID()
            instance().getStorageDiskManager(world as ServerWorld)!![id] = instance().createDefaultItemDisk(world as ServerWorld, getCapacity(stack))
    ...
    fun onItemRightClick(world: World, player: PlayerEntity, hand: Hand?): ActionResult<ItemStack> {
        ...
        if (!world.isClient && player.isCrouching() && type !== ItemStorageType.CREATIVE) {
            ...
            if (disk != null && disk.getStored() == 0) {
                val storagePart = ItemStack(StoragePartItem.Companion.getByType(type), diskStack.count)
    ...
    init {
        this.type = type
        this.setRegistryName(RS.ID, type.getName().toString() + "_storage_disk")
    }
}