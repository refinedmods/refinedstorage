...
class WirelessGridItem(val type: Type) : NetworkItem(Properties().group(RS.MAIN_GROUP).maxStackSize(1), type == Type.CREATIVE, Supplier { RS.SERVER_CONFIG.wirelessGrid.getCapacity() }) {
    enum class Type {
        NORMAL, CREATIVE
    }
    ...
    override fun provide(handler: INetworkItemManager?, player: PlayerEntity?, stack: ItemStack?, slotId: Int): INetworkItem? {
        return WirelessGridNetworkItem(handler!!, player!!, stack!!, slotId)
    }

    companion object {
        @kotlin.jvm.JvmStatic
        fun getViewType(stack: ItemStack): Int {
            return if (stack.hasTag() && stack.tag!!.contains(GridNetworkNode.NBT_VIEW_TYPE)) stack.tag!!.getInt(GridNetworkNode.NBT_VIEW_TYPE) else IGrid.VIEW_TYPE_NORMAL
        }
    ...
    init {
        this.setRegistryName(RS.ID, (if (type == Type.CREATIVE) "creative_" else "") + "wireless_grid")
    }
}