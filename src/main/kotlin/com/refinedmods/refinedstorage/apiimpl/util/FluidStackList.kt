...
class FluidInstanceList(
        private val stacks: ArrayListMultimap<Fluid, StackListEntry<FluidInstance>> = ArrayListMultimap.create<Fluid, StackListEntry<FluidInstance>>(),
    ...
    fun add(stack: FluidInstance, size: FluidValue): StackListResult<FluidInstance> {
        ...
        stacks[stack.fluid].forEach { entry ->
        ...
        stacks.put(newStack.fluid, newEntry)
    ...
    override operator fun get(stack: FluidInstance, flags: Int): FluidInstance? {
        for (entry in stacks[stack.fluid]) {
            val otherStack: FluidInstance = entry.stack
    ...
    fun getStacks(): Collection<StackListEntry<FluidInstance>> {
    ...
    override fun copy(): IStackList<FluidInstance> {
        ...
        for (entry in stacks.values()) {
            ...
            list.stacks.put(entry.stack.fluid, StackListEntry(entry.id, newStack))
